...
module.exports = function (payload) {
  ...
  return new Promise(function (resolve, reject) {
    self.r.table('campaigns').get(self.getIDfromURN(payload.urn))
      .then(function (campaign) {
        // TODO :: if ( campaign.publish===true ) return reject("campaign already published")
        campaign.publish = true
        campaign.urn = payload.urn
        return self.execute('command', 'campaignUpdateCommand', campaign)
          .then(function (updated) {
            return self.r.table('individuals').filter({}).hasFields(['charity'])
