...
module.exports = function (startup) {
  ...
  return new Promise(function (resolve, reject) {
    Promise.props({
      score: self.knex(database + '.smf_votes').sum('points as points').where({startup_id: startup}),
      name: self.knex(database + '.smf_votes').distinct('startup_name').where({startup_id: startup})
